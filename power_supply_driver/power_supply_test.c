/**
 * power supply driver for BQ24735
 */
...
#define BQ24735_CHARGE_VOLTAGE_MASK 0x7FC0
					...
					u16 mask, u16 value)
{
	...
	if (ret < 0) {
		dev_err(&client->dev, "failed to read bq24735: %d\n", ret);
		return ret;
	}
...
static int bq24735_config_charger(struct bq24735 *charger)
{
	...
	if (pdata->charge_current) {
		...
		ret = bq24735_write_word(charger->i2c_client,
						BQ24735_CHARGE_CURRENT, value);
		if (ret < 0) {
			dev_err(&charger->i2c_client->dev,
					"failed to write charger current: %d, return: %d\n",
					value, ret);
			return ret;
		}
	}
	...
	if (pdata->charge_voltage) {
		...
		if (ret < 0) {
			dev_err(&charger->i2c_client->dev,
					"failed to write charger voltage: %d, return: %d\n",
					value, ret);
			return ret;
		}
	}
	...
	if (pdata->input_current) {
		...
		if (ret < 0) {
			dev_err(&charger->i2c_client->dev,
					"failed to write input current: %d, return: %d\n",
					value, ret);
			return ret;
		}
	}
...
static inline int bq24735_enable_charging(struct bq24735 *charger)
{
	...
	return bq24735_update_word(charger->i2c_client, BQ24735_CHG_OPT,
...
static bool bq24735_charger_is_present(struct bq24735 *charger)
{
	if (charger->status_gpio) {
		...
		int ac = 0;
		ac = bq24735_read_word(charger->i2c_client, BQ24735_CHG_OPT);
		if (ac < 0) {
			dev_dbg(&charger->i2c_client->dev, 
					"failed to read charger option: %d\n",
					ac);
			return false;
		}
...
static int bq24735_charger_is_charging(struct bq24735 *charger)
{
	...
	ret = bq24735_read_word(charger->i2c_client, BQ24735_CHG_OPT);
	if (ret < 0) {
		dev_dbg(&charger->i2c_client->dev,
				"failed to read charger option: %d\n",
				ret);
...
static struct bq24735_platform *bq24735_parse_dt_data(struct i2c_client *client)
{
	...
	pdata = devm_kzalloc(&client->dev, sizeof(struct bq24735_platform), GFP_KERNEL);
	if (!pdata) {
	...
	ret = of_property_read_u32(np, "ti,charge-charge_voltage", &val);
	if (!ret)
							...
							const struct i2c_device_id *id)
{
	...
	charger = devm_kmalloc(&client->dev, sizeof(struct bq24735), GFP_KERNEL);
	if (!charger) {
		dev_err(&client->dev, "failed to malloc bq24735 dev\n");
		return -ENOMEM;
	}
	...
	if (!charger->pdata && client->dev.of_node)
	...
	if (!name) {
		name = devm_kasprintf(&client->dev, GFP_KERNEL, "bq24735&%s",
						dev_name(&client->dev));
		if (!name) {
	...
	if (IS_ERR(charger->status_gpio)) {
		...
		dev_err(&client->dev, "getting ac detect gpio faled: %d\n", ret);
		return ret;
	}
...
static int bq24735_charger_remove(struct i2c_client *client)
{
	...
	if (charger->poll_interval)
		cancel_delayed_work(&charger->poll);
